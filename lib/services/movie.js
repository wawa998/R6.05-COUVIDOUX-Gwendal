...
module.exports = class MovieService extends Service {
    ...
    async delete(movieId) {
        ...
        const mail = new Mail();
        const deletedMovie = await Movie.query().findById(movieId);
        const res = await Movie.query().deleteById(movieId);

        if (res === 1) {
            const userIds = await Favorite.query().select('id_user').where('id_movie','=', movieId);

            for (const userId of userIds) {

                const userMail = await User.query().select('mail').where('id','=', userId.id_user);

                mail.mailWhenFavIsDeleted(userMail[0].mail, deletedMovie.title);
            }
    ...
    async patch(id, movie) {
        ...
        for (const userId of userIds) {
            ...
            await mail.patchMovieNotification(userMail[0].mail, patchedMovie.title);
        }
